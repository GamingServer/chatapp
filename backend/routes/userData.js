...
router.post('/genOtp', async (req, res) => {
    ...
    if (phone) {
        await otpSave.updateOne({ phoneNumber: phoneNumber }, { $set: { otp: otp } });
...
router.post('/verifyOtp', async (req, res) => {
    ...
    if (otpVerify) {
        if (otpVerify.otp == otp) {
            ...
            return res.status(200).json({username:user.username,email:user.email,phoneNumber:user.phoneNumber,_id:user._id,message:'otp verify successfully'});
        }
        else {
