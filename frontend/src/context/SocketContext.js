    ...
    if (authUser || isAdmin) {
      ...
      if (authUser && isAdmin) {
        ...
        socket.on("lastMessage", (value) => {
          setLastMsg((prev) =>
            prev.map((msg) =>
              msg.reciver.username === value.message.sender.username ||
              msg.sender.username === value.message.sender.username
                ? value.message
                : msg
            )
          );
        });
        socket.on("rolechange", (vlaue) => {
          setChangeRole(vlaue);
          console.log("rolechange", vlaue);
        });
      }
      setSocket(socket);
      return () => socket.close();
