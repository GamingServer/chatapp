                      ...
                      <div className={`${item.senderName !== 'admin' ? 'bg-[#007bff] text-white' : 'bg-[#e4e6eb] text-black'} rounded-[10px] px-3 py-2 max-w-[100%] min-w-[30%]`}>
                        {
                          item.type === 'image' ? (
                            <img
                              ...
                              alt="sent"
                              onClick={() => setFullMedia({ type: 'image', url: `http://localhost:8080${item.message}` })}
                            ...
                            <video
                              ...
                              onClick={() => setFullMedia({ type: 'video', url: `http://localhost:8080${item.message}` })}
                            />
                          ) : /^https?:\/\/.*\.(jpeg|jpg|gif|png|webp)$/i.test(item.message) ? (
                            <img
                              src={item.message}
                              alt="sent"
                              onClick={() => setFullMedia({ type: 'image', url: item.message })}
                              className="w-20 h-20 object-cover cursor-pointer rounded"
                            />
                          ) : /^https?:\/\/.*\.(mp4|webm|ogg)$/i.test(item.message) ? (
                            <video
                              src={item.message}
                              controls
                              className="w-32 h-24 rounded cursor-pointer"
                              onClick={() => setFullMedia({ type: 'video', url: item.message })}
                            />
